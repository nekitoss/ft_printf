...
/*
 Возможно кому то еще актуально:)
 
 UPD:
 Теперь для каждого теста выводятся 3 строки
 1 - то что суется в принтф
 2 - то что печатает оригинальный принтф
 3 - то что печатает ваш принтф
 
 Если в строке 1 вы не видите буквы конвертации, а видите "CNV" - значит смотрите вывод чуть выше, там обязательно написана буква конвертации.
 
 okosiako [3:57 PM]
 printf_main.c лучше компилить с -Wno-format-invalid-specifier -Wno-format -Wno-macro-redefined -Wno-implicitly-unsigned-literal иначе будет очень много ворнингов. сообщения об ошибках будут видны
